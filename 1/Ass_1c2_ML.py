...
x = np.ones((b.shape[0],b.shape[1]+11))
x[:,:-11] = b
x[:,5] = np.multiply(x[:,0], x[:,0])
x[:,6] = np.multiply(x[:,1], x[:,1])
x[:,7] = np.multiply(x[:,2], x[:,2])
x[:,8] = np.multiply(x[:,3], x[:,3])
x[:,9] = np.multiply(x[:,0], x[:,1])
x[:,10] = np.multiply(x[:,0], x[:,2])
x[:,11] = np.multiply(x[:,0], x[:,3])
x[:,12] = np.multiply(x[:,1], x[:,2])
x[:,13] = np.multiply(x[:,1], x[:,3])
x[:,14] = np.multiply(x[:,2], x[:,3])
# print(type(b))
# <class 'numpy.ndarray'>
...
alpha_list = [0.01, 0.02, 0.05, 0.1, 0.2, 0.3, 0.5]
err = []
